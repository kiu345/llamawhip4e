...
public class AIChatViewPart extends ViewPart {
    ...
    public void initState() {
        ...
        if (tabFolder.getItemCount() == 0) {
            (active = addNewChatTab(true)).setSelectedModel(config.getDefaultModel());
        }
    ...
    public void createNewChat(String initialMessage) {
        // does not work right now
        var newChat = addNewChatTab(false);
        newChat.setSelectedModel(PluginConfiguration.instance().getDefaultModel());
        newChat.prepareDoSend(initialMessage, false);
    }

    public void sendInCurrentChat(String message) {
        if (active instanceof ChatComposite chatView) {
            chatView.removeInputElement();
            chatView.prepareDoSend(message, false);
        }
    }
    ...
    private void activateTab(int index) {
        active = chatComposites.get(index);
